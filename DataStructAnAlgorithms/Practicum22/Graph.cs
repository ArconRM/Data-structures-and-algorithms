...
public class Graph
{
    private class Node //вложенный класс для скрытия данных и алгоритмов
    {
        ...
        public int this[int i, int j] //матрица смежности
//индексатор для обращения к матрице смежности
        ...
        public int Size
        {
//свойство для получения размерности матрицы смежности
...
// пока очередь не пуста
                ...
                {
                    if (_array[v, u] != 0 && nov[u]) // смежные с данной и еще не просмотренные
                    {
...
// рекурсивно восстанавливаем путь между вершинами k и b
    ...
    {
//закрытое поле, реализующее АТД «граф»
...
//запускаем алгоритм обхода графа в глубину
        graph.Dfs(v);
        Console.WriteLine();
    }
    ...
    {
        graph.NovSet(); //помечаем все вершины графа как непросмотренные
//запускаем алгоритм обхода графа в ширину
        graph.Bfs(v);
        Console.WriteLine();
    }
...
//анализируем полученные данные и выводим их на экран
        Console.WriteLine("Длины кратчайших путей от вершины {0} до вершины", v);
        for (int i = 0; i < graph.Size; i++)
        {
            if (i != v)
            {
...
//анализируем полученные данные и выводим их на экран
    ...
    public void SearchG(int v0 = 0) //во внешнем классе
    {
        ...
        Stack<int> c = new();
        graph.SearchG(v0, ref a, ref c);
        
        if (c.Count == 0)
            Console.WriteLine("Эйлерового цикла в графе нет");
        else
            while (c.Count != 0)
                Console.Write("{0} ", c.Pop() + 1);
    }

    public void AreBothAdjacentWithThird(int v1, int v2, int v)
    {
        if (graph[v1, v] != 0 && graph[v2, v] != 0)
            Console.WriteLine($"Вершины {v1} и {v2} соседствуют с {v}");
        else
            Console.WriteLine($"Вершины {v1} и {v2} не соседствуют с {v}");
    }
}